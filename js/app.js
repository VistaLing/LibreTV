...
function addAdultAPI() {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS && (localStorage.getItem('yellowFilterEnabled') === 'false')) {
        ...
        adultTitle.innerHTML = `少儿不宜资源采集站 <span class="adult-warning">
...
function checkAdultAPIsSelected() {
    ...
    if (hasAdultSelected) {
        ...
        if (filterDescription) {
            filterDescription.innerHTML = '<strong class="text-pink-300">选中少儿不宜资源站时无法启用此过滤</strong>';
        }
        ...
        // 恢复原来的描述文字
        if (filterDescription) {
            filterDescription.innerHTML = '过滤"伦理片"等少儿不宜内容';
        }
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `SCUTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'SCUTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'SCUTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'SCUTV-Settings_' + times + '.json');
}
